			...
			<div class="col">
				<h2>
					...
					<span class="text-white">{{
						teamsPlaying.home.score
					}}</span>
				</h2>
			</div>
			<div v-if="gameStatus === 'Live'" class="col text-white">
				{{ gamePlayByPlay.slice(-1)[0].about.halfInning }}
				{{ gamePlayByPlay.slice(-1)[0].about.inning
				}}{{
		...
		<div v-if="playerEvents">
			<div class="row">
				<div class="col-md-5">
					<div v-for="player in battingOrder" :key="player">
						...
						>
							...
							>
								<i
									v-if="show === player.person.id"
									class="bx bx-caret-down me-2 text-white"
								></i>
								<i v-else class="bx bx-caret-right me-2"></i>
								<span class="color-sand me-2">
								...
								>
								<span
									v-for="curr in userItems[0].attributes
										.currentPlayers"
									:key="curr"
								>
									<i
				...
				<div v-if="!editPlayer" class="col-md-5 text-center">
					...
					>
						{{ userSelectedPlayersTotalPoints > 0 ? '+' : ''
						}}{{
							userSelectedPlayersTotalPoints !== null
