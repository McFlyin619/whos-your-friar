	...
	<div>
		...
		<div class="row mb-5 border-bottom-yellow text-center">
			...
			<div v-if="gameStatus === 'Live'" class="col text-white">
				...
				}}{{ gamePlayByPlay.slice(-1)[0].about.inning === 1 ? 'st' : gamePlayByPlay.slice(-1)[0].about.inning === 2 ? 'nd' : gamePlayByPlay.slice(-1)[0].about.inning === 3 ? 'rd' : 'th' }}
			</div>
			<div class="col">
				<h2>
			...
			<div class="row">
				<div class="col-md-5">
					<div v-for="player in battingOrder" :key="player">
						<a role="button" @click="show === player.person.id ? (show = null) : (show = player.person.id)">
							<p v-if="playerEvents[player.person.fullName.split('.').join('')]">
								...
								<span :class="{ 'text-danger': playerEvents[player.person.fullName.split('.').join('')].points > 10 }"
									><i v-if="playerEvents[player.person.fullName.split('.').join('')].points > 10" class="fa-solid fa-fire text-danger"></i>
								...
								<span
									...
									>{{ playerEvents[player.person.fullName.split('.').join('')].points > 0 ? '+' : '' }}{{ playerEvents[player.person.fullName.split('.').join('')].points }}</span
								>
						...
						<div v-if="show === player.person.id && playerEvents[player.person.fullName.split('.').join('')].id === player.person.id">
							<div class="text-white ms-4" v-for="play in playerEvents[player.person.fullName.split('.').join('')].plays.s" :key="play">
								<h6>
									<span class="color-sand">{{ play.about.halfInning.toUpperCase() }} {{ play.about.inning }}</span>
									&#183; {{ play.result.event }}
									<span v-if="play.result.rbi > 0"> - {{ play.result.rbi }} RBI{{ play.result.rbi > 1 ? "'s" : '' }}</span>
								</h6>
							</div>
							<div class="text-white ms-4" v-for="player in playerEvents[player.person.fullName.split('.').join('')].plays.b" :key="player">
								<h6>{{ player }}</h6>
							</div>
						</div>
					</div>
		...
		<div v-if="gameStatus === 'Live'">
			...
			<div v-for="play in gamePlayByPlay.slice().reverse()" :key="play">
				<h5 class="color-sand">
					{{ play.about.halfInning.toUpperCase() }} {{ play.about.inning }}{{ play.about.inning === 1 ? 'st' : play.about.inning === 2 ? 'nd' : play.about.inning === 3 ? 'rd' : 'th' }}
				</h5>
