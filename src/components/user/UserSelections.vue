<template>
	<div v-if="userItems" class="container mt-5">
		<base-modal :show="showEditForm" title="Are you sure you want to pick new players?" @close="closeForm">
			<edit-player :userItems="userItems" @close="closeForm"></edit-player>
		</base-modal>
		<form v-if="!userItems[0].attributes.currentPlayers" @submit.prevent="savePlayers">
			<div>
				<h1 v-if="!userItems[0].attributes.userTeam">Select a team</h1>
				<select v-if="!userItems[0].attributes.userTeam" v-model="team" class="form-select">
				...
				<select :disabled="player3 !== null" v-if="player1 !== null" v-model="player2" class="form-select mb-4">
					<option selected :value="null">Choose a position player</option>
					<option v-for="player in positionRoster2" :key="player" :value="player">
						{{ player.person.fullName }}
					</option>
				</select>
				<select v-if="player2 !== null" v-model="player3" class="form-select mb-4">
					<option selected :value="null">Choose a position player</option>
					<option v-for="player in positionRoster3" :key="player" :value="player">
						{{ player.person.fullName }}
					</option>
				</select>
			...
			<div v-if="player3 !== null" class="mt-2">
				<button @click="savePlayers" :disabled="isLoading" class="btn sd-btn me-4">
			...
			<div>
				<div v-for="(player, index) in previousPlayers" :key="player">
					<p class="text-white text-start">
						<span class="color-sand" v-if="index === 0 || index === 1 || index === 2">Week 1</span>
						<span class="color-sand" v-if="index === 3 || index === 4 || index === 5">Week 2</span>
						<span class="color-sand" v-if="index === 6 || index === 7 || index === 8">Week 3</span>
						{{ player.person.fullName }}
					</p>
				</div>
			</div>
