		...
		<div class="mt-5">
			...
			<div v-if="gameStatus === 'live'" class="table-responsive">
				<table class="table color-yellow text-center mt-5">
					<thead>
						<tr class="">
							<th class="text-start" scope="col" @click="show === null ? (show = 'all') : (show = null)"> <i v-if="show === 'all'" class="fas fa-caret-down me-2 text-white"></i>
								<i v-else class="fas fa-caret-right color-sand me-2"></i>Pos</th>
							...
							<th class="text-center" scope="col">Total Points</th>
						</tr>
					</thead>
					<tbody>
						<tr
							...
							:key="user"
							:class="{
								'text-primary': page === 1 ? (index + 1) === 1 : ((page - 1)*perPage) + (index + 1) === 1,
								'text-info': page === 1 ? (index + 1) === 2 : ((page - 1)*perPage) + (index + 1) === 2,
								'text-white': page === 1 ? (index + 1) === 3 : ((page - 1)*perPage) + (index + 1) === 3,
								'user-standing': currentUser.username === user.user.attributes.userName
							}"
							role="button"
							@click="show === null ? (show = user.user) : (show = null)"
						>
							<td class="color-yellow text-start">
								<i v-if="show === user.user || show === 'all'" class="fas fa-caret-down me-2 text-white"></i>
								<i v-else class="fas fa-caret-right color-sand me-2"></i>
								{{ page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1) }}
							</td>
							<td class="text-start">
								<span
									class="ms-2 text-danger d-flex"
									v-if="(page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1)) > user.user.attributes.currentPosition"
								>
									<i class="fa-solid fa-caret-down align-self-center"></i>
									{{ (page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1)) - user.user.attributes.currentPosition }}
								</span>
								<span
									class="ms-2 text-success d-flex"
									v-if="(page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1)) < user.user.attributes.currentPosition"
								>
									<i class="fa-solid fa-caret-up align-self-center"></i>
									{{ user.user.attributes.currentPosition - (page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1)) }}
								</span>
								<span class="color-sand" v-if="(page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1)) === user.user.attributes.currentPosition"
									><i class="fa-solid fa-minus"></i
								></span>
							</td>
							<td></td>
							<td class="text-start">
								<i v-if="(page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1)) < 4" class="fa-solid fa-star"></i>
								...
								<span
									...
									>{{ numbers.numbers[user.user.attributes.favPlayerNumber] }}</span
								>
								<i v-if="(page === 1 ? (index + 1) : ((page - 1)*perPage) + (index + 1)) < 4" class="fa-solid fa-star ms-2"></i>
								<div
									v-if="
										!editPlayerSelection &&
										user.user.attributes.currentPlayers !== null &&
										(show === user.user || show === 'all')
									"
								>
									<div class="color-sand ms-2" v-for="players in user.players" :key="players">
										{{ players.name }} &#183;
										<span class="text-success"
											>{{ players.points > 0 ? '+' : '' }}{{ players.points }}</span
										>
									</div>
								</div>
							</td>
							<td class="text-center text-success">
								{{ user.gamePoints > 0 ? '+' : '' }}{{ user.gamePoints }}
							</td>
							<td class="text-center text-success">{{ user.total > 0 ? '+' : '' }}{{ user.total }}</td>
						</tr>
					</tbody>
				</table>
			</div>
			<div v-else class="table-responsive">
				<table class="table color-yellow text-center mt-5">
					<thead>
						<tr class="">
							<th role="button" class="text-start" scope="col" @click="show === null ? (show = 'all') : (show = null)"> <i v-if="show === 'all'" class="fas fa-caret-down me-2 text-white"></i>
								<i v-else class="fas fa-caret-right color-sand me-2"></i> Pos</th>
							<th class="text-start" scope="col">Change</th>
							<th scope="col"></th>
							<th class="text-start" scope="col">User</th>
					...
					<tbody>
						<tr
							...
							:key="user"
							:class="{
								'text-primary': user.attributes.currentPosition === 1,
								'text-info': user.attributes.currentPosition === 2,
								'text-white': user.attributes.currentPosition === 3,
								'user-standing': currentUser.username === user.attributes.userName
							}"
							role="button"
							@click="show === null ? (show = user) : (show = null)"
						>
							<td class="color-yellow text-start">
								<i v-if="show === user || show === 'all'" class="fas fa-caret-down me-2 text-white"></i>
								<i v-else class="fas fa-caret-right color-sand me-2"></i>
							...
							<td class="text-start">
								<span
									class="ms-2 text-danger d-flex"
									v-if="user.attributes.currentPosition > user.attributes.previousPosition"
								>
									<i class="fa-solid fa-caret-down align-self-center"></i>
									{{ user.attributes.currentPosition - user.attributes.previousPosition }}
								</span>
								<span
									class="ms-2 text-success d-flex"
									v-if="user.attributes.currentPosition < user.attributes.previousPosition"
								>
									<i class="fa-solid fa-caret-up align-self-center"></i>
									{{ user.attributes.previousPosition - (user.attributes.currentPosition) }}
								</span>
								<span class="color-sand" v-if="user.attributes.currentPosition === user.attributes.previousPosition"
									><i class="fa-solid fa-minus"></i
								></span>
							</td>
							<td></td>
							<td class="text-start">
								<i v-if="user.attributes.currentPosition < 4" class="fa-solid fa-star me-2"></i>
								...
								<span
									...
									class="yellow-circle-border text-white favNum"
									>{{ numbers.numbers[user.attributes.favPlayerNumber] }}</span
								>
								<i v-if="user.attributes.currentPosition < 4" class="fa-solid fa-star ms-2"></i>
								<div
									v-if="
										!editPlayerSelection && user.attributes.currentPlayers !== null && (show === user || show === 'all')
									"
								>
									<div
										class="color-sand ms-2"
										v-for="players in user.attributes.currentPlayers"
										:key="players"
									>
							...
							<td class="text-center text-success">
								{{ user.attributes.userPoints > 0 ? '+' : '' }}{{ user.attributes.userPoints }}
							</td>
						</tr>
					</tbody>
				</table>
			...
			<div class="d-flex justify-content-between">
				<div>
					<i role="button" v-show="page !== 1" @click="pageDown" class="fa-solid fa-square-caret-left fa-2x"></i>
				</div>
				<div>
					<i role="button" v-show="page !== Math.ceil(standings.length / perPage)" @click="pageUp" class="fa-solid fa-square-caret-right fa-2x"></i>
				</div>
			</div>
			<small class="text-white"
				>***Click a players name to view thier selected players. Viewing other players selections will be
				available when this weeks first game starts.
			</small>
		</div>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		standings () {
			if (this.currentUser && this.$store.state.standings) {
				const order = this.$store.state.standings
					.filter(
						(i) =>
							i.attributes.userTeam.id ===
							(this.team !== null ? this.team.id : this.userItems[0].attributes.userTeam.id)
					)
				return order.sort((a, b) => a.attributes.currentPosition - b.attributes.currentPosition)
		...
		pagedStandings () {
			return this.standings.slice((this.page - 1) * this.perPage, this.page * this.perPage)
		},
		getCurrentPoints () {
			const usersPoints = []
